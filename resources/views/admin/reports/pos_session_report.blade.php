...
<main class="progga-content">
    ...
    <div class="progga-card mb-3">@include('admin.reports.partials.filter_component',['showAllFilterType'=>true,'showSearchFilter'=>true,'searchPlaceholder'=>'Search session, employee, status...'])</div>
    <div class="progga-card" id="posSessionReportCard" data-report-card>
        ...
        <div class="report-table-shell"><table class="progga-table enhanced-report-table"><thead><tr><th>SL</th><th>Employee</th><th>Day</th><th>Start Time</th><th>End Time</th><th>Duration</th><th>Grand Total</th><th>Status</th><th>Action</th></tr></thead><tbody id="posSessionReportRows">@include('admin.reports.partials.pos_session_report_rows')</tbody></table></div>
        <div id="posSessionReportPagination">@include('admin.reports.partials.custom_pagination',['paginator'=>$sessions])</div>
    </div>
</main>
@endsection
@section('script')
<script>
window.updateReportDOM=function(data){$('#posSessionReportRows').html(data.html||'');$('#posSessionReportPagination').html(data.pagination||'');};
